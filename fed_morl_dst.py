...
class client_local(object):
    def __init__(self,id,preference,env):
        ...
        self.agent = MetaAgent(model, args, is_train=True)
        self.id=id
    ...
    def local_train(self,args):
        ...
        for num_eps in range(args.eps_per_round):
            ...
            while not terminal:
                ...
                self.agent.model, self.agent.model_, loss =self.agent.learn()
...
def fed_train(args,global_client,client_list):
    ...
    for round_num in range(args.round):
        ...
        for client_ in Client_List:
            client_.agent.model.load_state_dict(global_client.agent.model.state_dict())
            client_.agent.model_.load_state_dict(global_client.agent.model_.state_dict())
        ...
        # print()
        critic_model_temp_list =[client_temp.agent.model_ for client_temp in output_list]
        critic_model_list = [copy.deepcopy(model) for model in critic_model_temp_list]
        criti_model_temp_list = [client_temp.agent.model for client_temp in output_list]
        ...
        ###########进行模型聚合
            ...
            global_client.agent.model_.load_state_dict(fed_state_dict)
            ...
            #### update fed weights to fl model
            global_client.agent.model.load_state_dict(fed_state_dict)
        ...
        if (round_num + 1) % args.round == 0:
            with open('0506_fed_train_dst.txt', 'a') as f:  # 设置文件对象
                f.write(
                    'fed_MORL_dst_sample_ratio#######################################################################################')
                f.write('\n')
                f.write('client_num=')
        ...
        if (round_num + 1) % 100 == 0:
            ...
            global_client.agent.save(args.save,
                                     "m.{}_e.{}_n.{}_r.{}".format(args.model, args.env_name, args.critic_name,
                                                                  round_num))
    ...
    global_client.agent.save(args.save, "m.{}_e.{}_n.{}".format(args.model, args.env_name, args.critic_name))
...
if __name__ == '__main__':
    ...
    if args.serialize:
        ...
        modle = get_new_model(args.model, state_size, action_size, reward_size)
