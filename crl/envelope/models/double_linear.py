...
class EnvelopeLinearCQN(torch.nn.Module):
    ...
    def __init__(self, state_size, action_size, reward_size):
        ...
        self.affine6 = nn.Linear(state_size + reward_size,
                                 (state_size + reward_size) * 16)
        self.affine7 = nn.Linear((state_size + reward_size) * 16,
                                 (state_size + reward_size) * 32)
        self.affine8 = nn.Linear((state_size + reward_size) * 32,
                                 (state_size + reward_size) * 64)
        self.affine9 = nn.Linear((state_size + reward_size) * 64,
                                 (state_size + reward_size) * 32)
        self.affine10 = nn.Linear((state_size + reward_size) * 32,
                                 action_size * reward_size)
    ...
    def H(self, Q, w, s_num, w_num):
        ...
        prod = torch.bmm(reQ_ext.cpu().unsqueeze(1), w_ext.cpu().unsqueeze(2)).squeeze()
    ...
    def forward(self, state, preference, w_num=1):
        ...
        x1 = torch.cat((state, preference), dim=1)
        x1 = x1.view(x1.size(0), -1)
        x1 = F.relu(self.affine6(x1))
        x1 = F.relu(self.affine7(x1))
        x1= F.relu(self.affine8(x1))
        x1 = F.relu(self.affine9(x1))
        q1 = self.affine10(x1)
        q1 = q1.view(q.size(0), self.action_size, self.reward_size)
        hq1 = self.H(q1.detach().view(-1, self.reward_size), preference, s_num, w_num)
        return hq, q, hq1, q1




class MO_ActorDiscrete(torch.nn.Module):
    """
    :param env: gym environment
    """
    def __init__(self, state_size, action_size, reward_size):
        super(MO_ActorDiscrete, self).__init__()
        # self.env = env
        self.state_size = state_size
        self.action_size = action_size
        self.reward_size = reward_size
        self.lin1 = nn.Linear(self.state_size+self.reward_size, 256)
    ...
    def forward(self, state, preference):
        ...
        # print(state[0],preference.shape)
        x = torch.cat((state, preference), dim=-1) ##########横向拼接
        x = x.view(-1,x.size(-1))
    ...
    def action(self, state,preference):
        """
        :param state: (ds,)
        :return: an action
        """
        with torch.no_grad():
            p = self.forward(state,preference)
